defmodule Transformerex.Path do
  ...
  def transform(mapping: [mapping], path: path, target: _target, json: json) do
    TransList.transform(path: path, mapping: mapping, json: json)
  end

  def transform(mapping: mapping, path: path, target: _target, json: json)
      when is_bitstring(mapping) do
    if String.match?(mapping, ~r/\A(\$\.)?[[:alpha:]_\-]+\z/) do
      Root.transform(path: path, target: mapping, json: json)
    end
  end

  def transform(mapping: mapping, path: path, target: _target, json: json) when is_map(mapping) do
